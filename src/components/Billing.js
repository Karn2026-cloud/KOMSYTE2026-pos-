...
// The user prop is now optional, which is key to the fix.
export default function Billing({ user }) {
  ...
  // ✅ FIXED: Use optional chaining (?.) to safely access the plan.
  ...
  const generateAndDownloadPDF = () => {
    const doc = new jsPDF();
    doc.setFontSize(18);
    // Use optional chaining for shopName as well, just in case
    doc.text(user?.shop?.shopName || "Your Store", 105, 20, { align: 'center' });
    doc.setFontSize(10);
